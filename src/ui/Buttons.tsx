...
import { deleteEmployeeById } from "@/app/lib/actions";
...
export const SubmitButton = ({ title }: { title: string }) => {
  ...
  return (
    <button
      ...
      className="rounded-md w-20 bg-green-500 hover:shadow-lg hover:shadow-green-500/50 text-white disabled:bg-slate-400"
    >
      {pending ? (
        <div role="status">
          <svg
            aria-hidden="true"
            className="inline w-4 h-4 text-green-100 animate-spin fill-green-400"
            viewBox="0 0 100 101"
            fill="none"
...
export const UpdateEmployeeButton = ({ id }: { id: string }) => {
  return (
    <Link
...
export const DeleteEmployeeButton = ({ id }: { id: string }) => {
  const deleteEmployee = deleteEmployeeById.bind(null, id);
  ...
  return (
    ...
    >
      <button name="delete" className="hover:bg-red-50 rounded-md border p-2">
        <svg
          xmlns="http://www.w3.org/2000/svg"
          fill="none"
          viewBox="0 0 24 24"
